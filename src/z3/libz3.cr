...
lib LibZ3
  ...
  fun get_seq_sort_basis = Z3_get_seq_sort_basis(ctx : Context, sort : Sort) : Sort
  fun get_sort = Z3_get_sort(ctx : Context, ast : Ast) : Sort
  fun get_sort_kind = Z3_get_sort_kind(ctx : Context, sort : Sort) : SortKind
  fun get_string_contents = Z3_get_string_contents(ctx : Context, ast : Ast, length : UInt32, contents : UInt32*) : Void
  fun get_string_length = Z3_get_string_length(ctx : Context, ast : Ast) : UInt32
  ...
  fun is_string = Z3_is_string(ctx : Context, ast : Ast) : Bool
  ...
  fun mk_int_to_str = Z3_mk_int_to_str(ctx : Context, a : Ast) : Ast
  ...
  fun mk_sbv_to_str = Z3_mk_sbv_to_str(ctx : Context, bv : Ast) : Ast
  fun mk_seq_at = Z3_mk_seq_at(ctx : Context, s : Ast, index : Ast) : Ast
  fun mk_seq_concat = Z3_mk_seq_concat(ctx : Context, count : UInt32, asts : Ast*) : Ast
  fun mk_seq_contains = Z3_mk_seq_contains(ctx : Context, container : Ast, containee : Ast) : Ast
  fun mk_seq_empty = Z3_mk_seq_empty(ctx : Context, sort : Sort) : Ast
  fun mk_seq_extract = Z3_mk_seq_extract(ctx : Context, s : Ast, offset : Ast, length : Ast) : Ast
  fun mk_seq_index = Z3_mk_seq_index(ctx : Context, s : Ast, substr : Ast, offset : Ast) : Ast
  fun mk_seq_last_index = Z3_mk_seq_last_index(ctx : Context, s : Ast, substr : Ast) : Ast
  fun mk_seq_length = Z3_mk_seq_length(ctx : Context, s : Ast) : Ast
  fun mk_seq_nth = Z3_mk_seq_nth(ctx : Context, s : Ast, index : Ast) : Ast
  fun mk_seq_prefix = Z3_mk_seq_prefix(ctx : Context, prefix : Ast, s : Ast) : Ast
  fun mk_seq_replace = Z3_mk_seq_replace(ctx : Context, s : Ast, src : Ast, dst : Ast) : Ast
  fun mk_seq_replace_all = Z3_mk_seq_replace_all(ctx : Context, s : Ast, src : Ast, dst : Ast) : Ast
  fun mk_seq_sort = Z3_mk_seq_sort(ctx : Context, sort : Sort) : Sort
  ...
  fun mk_string_from_code = Z3_mk_string_from_code(ctx : Context, a : Ast) : Ast
  fun mk_string_sort = Z3_mk_string_sort(ctx : Context) : Sort
  fun mk_string_symbol = Z3_mk_string_symbol(ctx : Context, s : UInt8*) : Symbol
  fun mk_string_to_code = Z3_mk_string_to_code(ctx : Context, a : Ast) : Ast
  fun mk_sub = Z3_mk_sub(ctx : Context, count : UInt32, asts : Ast*) : Ast
  fun mk_true = Z3_mk_true(ctx : Context) : Ast
  fun mk_u32string = Z3_mk_u32string(ctx : Context, len : UInt32, chars : UInt32*) : Ast
  fun mk_ubv_to_str = Z3_mk_ubv_to_str(ctx : Context, bv : Ast) : Ast
